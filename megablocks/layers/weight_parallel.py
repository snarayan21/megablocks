...
class MemoryOptimizedWeightParallelMLPInt8Comm(torch.autograd.Function):
    ...
    @staticmethod
    @torch.cuda.amp.custom_fwd
    def forward(ctx, x, w1, w2, topo, group, args):
        # x: [m, k], w1: [n, k], w2: [n, k]
        if (not x.is_contiguous() or not w1.is_contiguous() or
            not w2.is_contiguous()):
            raise ValueError("Expected contiguous 'x', 'w1' and 'w2'.")
        ...
        # Layer 0: x @ w1.t().
        # Compress w1 to int8 for the all-gather
        parallel_w1, _ = _gather_weights(w1.to(torch.int8), group)
        # Uncompress parallel_w1 to the appropriate training datatype
        parallel_w1 = parallel_w1.to(common.dtype(args))
        sdd_out = stk.ops.sdd(x, parallel_w1.t(), topo)
        ...
        #
        # NOTE: Reuse the buffer for the w1 weight gather.
        # Compress w2 to int8 for the all-gather
        parallel_w2, _ = _gather_weights(w2.to(torch.int8), group, parallel_w1)
        # Uncompress parallel_w1 to the appropriate training datatype
        parallel_w2 = parallel_w2.to(common.dtype(args))
        dsd_out = stk.ops.dsd(gelu_out, parallel_w2)
